...
public class PageObject {
    ...
    // not to throw an error right away, but wait for the items to load
    ...
    //if try didn't work, then catch
    //a beautiful way instead of element1.sendKeys("test")
    //how it works: sendTextToWebElement(element1, "text")
    public void sendTextToWebElement(WebElement element, String text) {
        try {
            element.clear();
            element.sendKeys(text);
            System.out.println("LOGGING: Send the text: *** " + text + " *** to element: " + cutXpathString(element));
        } catch (Exception e) {
    ...
    public void selectElement(WebElement element) {
        try {
            element.isSelected();
            element.click();
            System.out.println("LOGGING: Selected element: " + cutXpathString(element));
        } catch (Exception e) {
            System.out.println("LOGGING: Waiting for " + cutXpathString(element) + " to be visible!");
            waitForElementToBeVisible(driver, WAIT_TIMEOUT(), element);
            element.isSelected();
    ...
    public void clickBack() {
        ...
        System.out.println("LOGGING: clicked the browser back button");
    }
    ...
    public boolean isElementDisplayed(WebElement element) {
        boolean isDisplayed;
        try {
            System.out.println("LOGGING: checking if element is displayed: " + cutXpathString(element));
            waitForElementToBeVisible(driver, WAIT_TIMEOUT(), element);
            isDisplayed = element.isDisplayed();
        } catch (Exception e) {
            System.out.println("LOGGING: Caught exception, checking if element is displayed: " + cutXpathString(element));
            waitForElementToBeVisible(driver, WAIT_TIMEOUT(), element);
            isDisplayed = element.isDisplayed();
        }
        return isDisplayed;
     ...
     */
    public static void waitForElementListToBeEmpty(WebDriver driver, int seconds, List<WebElement> elements) {
        WebDriverWait wait = (WebDriverWait) new WebDriverWait(driver, seconds)
                .ignoring(StaleElementReferenceException.class);
